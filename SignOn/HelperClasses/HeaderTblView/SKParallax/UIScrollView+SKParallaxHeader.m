...
#import "UIScrollView+SKParallaxHeader.h"

#import <QuartzCore/QuartzCore.h>

@interface SKParallaxView ()

@property (nonatomic, readwrite) SKParallaxTrackingState state;
...
@implementation UIScrollView (SKParallaxHeader)
...
- (void)addParallaxWithImage:(UIImage *)image andHeight:(CGFloat)height andShadow:(BOOL)shadow {
    ...
    {
        SKParallaxView *parallaxView = [[SKParallaxView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width*2, height) andShadow:shadow];
...
- (void)addParallaxWithView:(UIView*)view andHeight:(CGFloat)height andShadow:(BOOL)shadow {
    ...
    {
        SKParallaxView *parallaxView = [[SKParallaxView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, height) andShadow:shadow];
        [parallaxView setClipsToBounds:YES];
...
- (void)setParallaxView:(SKParallaxView *)parallaxView {
    objc_setAssociatedObject(self, &UIScrollViewParallaxView,
                             parallaxView,
                             OBJC_ASSOCIATION_ASSIGN);
}

- (SKParallaxView *)parallaxView {
...
@implementation SKParallaxShadowView
...
- (id)initWithFrame:(CGRect)frame andShadow:(BOOL)shadow {
    if(self = [super initWithFrame:frame]) {
        ...
        [self setState:SKParallaxTrackingActive];
        ...
        if (shadow) {
            self.shadowView = [[SKParallaxShadowView alloc] init];
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    ...
    if (contentOffset.y > 0) {
        [self setState:SKParallaxTrackingInactive];
    } else {
        [self setState:SKParallaxTrackingActive];
    }
    
    if(self.state == SKParallaxTrackingActive) {
